...
class DataCube {
 public:
     ...
     */
    explicit DataCube(array<size_t, DIM> sizes) : dimension(DIM), size(sizes) {
     ...
     */
    DataCube(array<size_t, DIM> sizes, vector<T> values) : DataCube(sizes) {
     ...
     */
    inline bool operator==(const DataCube &other) const {
        bool isEqual = true;
